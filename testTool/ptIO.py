...
Description: point cloud IO interface, works with ptio_backend.so
Date: 2025-12-06 18:00:01
'''
import numpy as np
import ctypes
import os
...
class PCC_IO:
    ...
    def _define_function_prototypes(self):
        ...
        self.lib.pcc_io_read.argtypes = [ctypes.c_void_p, ctypes.c_char_p, ctypes.c_double]
        self.lib.pcc_io_read.restype = ctypes.c_bool

        self.lib.pcc_io_write.argtypes = [ctypes.c_void_p, ctypes.c_char_p, ctypes.c_double, ctypes.c_bool]
        ...
        self.lib.pcc_io_get_points.argtypes = [ctypes.c_void_p, ctypes.POINTER(ctypes.POINTER(ctypes.c_double)), ctypes.POINTER(ctypes.c_size_t)]
        self.lib.pcc_io_get_points.restype = ctypes.c_bool

        self.lib.pcc_io_get_colors.argtypes = [ctypes.c_void_p, ctypes.POINTER(ctypes.POINTER(ctypes.c_uint8)), ctypes.POINTER(ctypes.c_size_t)]
        self.lib.pcc_io_get_colors.restype = ctypes.c_bool

        self.lib.pcc_io_get_reflectance.argtypes = [ctypes.c_void_p, ctypes.POINTER(ctypes.POINTER(ctypes.c_uint16)), ctypes.POINTER(ctypes.c_size_t)]
        self.lib.pcc_io_get_reflectance.restype = ctypes.c_bool

        self.lib.pcc_io_set_points.argtypes = [ctypes.c_void_p, ctypes.POINTER(ctypes.c_double), ctypes.c_size_t]
        self.lib.pcc_io_set_points.restype = ctypes.c_bool

        self.lib.pcc_io_set_colors.argtypes = [ctypes.c_void_p, ctypes.POINTER(ctypes.c_uint8), ctypes.c_size_t]
        self.lib.pcc_io_set_colors.restype = ctypes.c_bool

        self.lib.pcc_io_set_reflectance.argtypes = [ctypes.c_void_p, ctypes.POINTER(ctypes.c_uint16), ctypes.c_size_t]
    ...
    def get_points(self):
        ...
        success = self.lib.pcc_io_get_points(self.obj, ctypes.byref(points_ptr), ctypes.byref(point_count))
    ...
    def get_colors(self):
        ...
        success = self.lib.pcc_io_get_colors(self.obj, ctypes.byref(colors_ptr), ctypes.byref(point_count))
    ...
    def get_reflectance(self):
        ...
        success = self.lib.pcc_io_get_reflectance(self.obj, ctypes.byref(reflectance_ptr), ctypes.byref(point_count))
        if not success or point_count.value == 0:
            return np.zeros(0, dtype=np.uint16)
...
def pcwrite(file_path: str, points: np.ndarray, attribute: np.ndarray = None, colors: np.ndarray = None, reflectance: np.ndarray = None, asAscii: bool = False):
    ...
    if os.path.dirname(file_path) != '': os.makedirs(os.path.dirname(file_path), exist_ok=True)
...
if __name__ == '__main__':
    file = "longdress_vox10_1051.ply"
    p, c = pcread(file)
    pcwrite("test.ply", p, c, asAscii=True)
    import open3d as o3d
    pc_o3d = o3d.io.read_point_cloud("test.ply")
    c2 = np.asarray(pc_o3d.colors) * 255
    assert (c == c2).all()

    pc_o3d = o3d.io.read_point_cloud(file)
    c3 = np.asarray(pc_o3d.colors) * 255
    assert (c == c3).all()
