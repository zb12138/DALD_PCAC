...
lib = cdll.LoadLibrary(os.path.dirname(os.path.abspath(
    __file__))+'/resAc.so')  # class level loading lib
lib.encoding.restype = c_void_p
lib.encoding.argtypes = [POINTER(c_int32), c_int, c_int, POINTER(
    c_uint8), POINTER(c_uint32), c_bool]
lib.decoding.restype = c_void_p
lib.decoding.argtypes = [
    POINTER(c_uint8), c_int, c_int, c_int, POINTER(c_int32)]
