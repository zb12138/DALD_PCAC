...
def encode(symbols, pdf, binPath, useGpu=True, interaction=False):
    ...
    bin_sz_bit = GPU_AC.encode(
        filebin, symb_ptr, cnt_ptr, cnt.shape[0], cnt.shape[1], useGpu, interaction)
    return bin_sz_bit, time.time()-t
...
def decode(pdf, binPath, useGpu=True, interaction=False):
    ...
    if useGpu:
        ...
        cnt = cnt.cpu()
    symb_dec_ptr = ctypes.cast(
        symbols_dec.data_ptr(), ctypes.POINTER(ctypes.c_uint16))
    cnt_ptr = ctypes.cast(cnt.data_ptr(), ctypes.POINTER(ctypes.c_uint16))
    GPU_AC.decode(symb_dec_ptr, filebin, cnt_ptr, symb_num,
                  cnt.shape[1], useGpu, interaction)
    return symbols_dec, time.time()-t
